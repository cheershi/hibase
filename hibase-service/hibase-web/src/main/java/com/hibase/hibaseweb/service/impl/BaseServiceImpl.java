package com.hibase.hibaseweb.service.impl;
 ...
 */
@Slf4j
public abstract class BaseServiceImpl<M extends BaseMapper<T>, T extends BasePo> implements BaseService<T> {
    ...
    public T store(T entity) {
        ...
        HibaseAssert.isFalse(result.hasErrors(),result.getDefaultMessage());
                ...
                if (uniqueKey != null) {
                    ...
                    try {
                        ...
                        if (!temp.containsKey(uniqueKey.group())) {
                            ...
                            HibaseAssert.isFalse(value == null || StrUtil.isBlank(value.toString()),"处理唯一key失败，" + info.getProperty() + "不能为空");
                        ...
                        throw new HibaseException("处理唯一key失败", e);
                    }
                }
            }
    ...
    private void settingInfo(T entity) {
        ...
        BasePermVO basePermDO = null;

        if (entity instanceof BasePermVO) {

            basePermDO = (BasePermVO) entity;
        }
        if (StrUtil.isBlank(entity.getId())) {
            ...
            entity.setCreatedBy(UserContextHolder.getInstance().getUsername());
        }

        entity.setUpdatedBy(UserContextHolder.getInstance().getUsername());
    ...
    public T getById(Serializable id, boolean throwException) {
        ...
        if (t == null && throwException) {
            throw new HibaseException("记录不存在");
        }
        return t;
    }
